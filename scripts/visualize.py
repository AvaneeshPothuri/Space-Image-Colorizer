...
val_gray_files = sorted(os.listdir(val_gray_dir))
...
input_tensor = torch.FloatTensor(gray_resized[None, None, ...]).to(device)  # Shape: (1,1,256,256)

# Predict ab channels
with torch.no_grad():
    pred_ab = model(input_tensor).cpu().numpy()[0]  # Shape: (2,256,256)
